...
Changelog:
- November 13, 2025. **Service Worker Auto-Reload Fix**: Fixed critical bug where page automatically reloaded twice after 30-60 seconds without user interaction on production. Root cause: Service worker registration code checked for updates every 60 seconds and triggered `window.location.reload()` when new worker activated, combined with aggressive `skipWaiting()` and `clients.claim()` causing immediate activation. Solution: (1) Removed automatic `window.location.reload()` from service worker update handler in `client/src/registerServiceWorker.ts`, (2) Changed update check interval from 60 seconds to 24 hours, (3) Removed `skipWaiting()` and `clients.claim()` from `client/public/sw.js` to let service worker activate naturally on next page load, (4) Changed service worker registration to production-only in `client/src/main.tsx`. Testing confirmed: Page remains stable for 90+ seconds with zero automatic reloads while service worker continues to cache images correctly. Service worker updates now take effect on next manual page navigation instead of forcing immediate reload.
- November 4, 2025. **Pure CSS ScrollReveal Animation System**: Completely replaced framer-motion with pure CSS animations to achieve professional-grade smoothness. Root cause: framer-motion library overhead and JavaScript-driven animations were causing choppy, stuttering card reveals especially on mobile. Solution: (1) Replaced ScrollReveal.tsx with lightweight IntersectionObserver that simply adds CSS class when element enters viewport - zero JavaScript animation overhead, (2) Implemented pure CSS transform-based transitions in index.css with !important to ensure proper application, (3) Animations now use only transform property (translateX/translateY 20px) with cubic-bezier easing, (4) Hardware acceleration via translate3d(0,0,0) and translateZ(0), (5) CSS will-change managed automatically by browser, (6) Configurable duration/delay via inline styles. Benefits: Native browser optimization, GPU-accelerated by default, no JavaScript execution during animations, consistent professional smoothness across all devices. User confirmed animations now work smoothly with no stuttering.
- October 11, 2025. **Advanced Image Caching Implementation (Phase 1 & 2)**: Implemented comprehensive caching solution to eliminate slow image loading on cold starts. **Phase 1 - HTTP Caching**: Created `server/cache-middleware.ts` setting Cache-Control headers (max-age=31536000, immutable) for all static assets - images cached for 1 year in browser disk storage. Updated `server/index.ts` to apply caching middleware in both dev and production. **Phase 2 - Service Worker**: Implemented aggressive client-side caching with cache-first strategy. Created `client/public/sw.js` service worker that precaches critical assets (hero image, logo) on first visit and serves all images from cache storage instantly. Added `client/src/registerServiceWorker.ts` for SW registration and lifecycle management. Testing confirmed: Cache-Control headers present on all assets, images load from cache on refresh (transferSize=0), service worker active with critical assets precached. Result: Images now load instantly even after browser restart, solving the cold-start performance issue on both Chrome and Safari.
- October 11, 2025. **Safari Mobile Tan Strip and Performance Fixes**: Fixed tan strip at top by removing Replit dev banner script from index.html. Optimized critical images with WebP: logo 197KB→21KB (89% reduction), hero 407KB→342KB (16% reduction). Updated Logo and Hero components to use picture elements with WebP sources and proper fallbacks. Added fetchpriority="high" and decoding="async" to critical images for faster Safari mobile loading. Fixed React fetchPriority prop warning by using lowercase attribute.
