...
Changelog:
- November 4, 2025. **ScrollReveal Animation Performance Optimization**: Fixed choppy card slide-in animations on mobile devices. Root cause: framer-motion animations combining opacity + transform transitions were too complex for mobile hardware, causing stuttering and rapid jumping during card reveals. Solution: (1) Removed opacity transitions entirely - now only animating transform for 50% reduction in compositor workload, (2) Reduced animation distance from 50px to 20px for smoother motion, (3) Reduced duration from 0.6s to 0.4s for snappier feel, (4) Simplified easing to cubic-bezier [0.25, 0.1, 0.25, 1], (5) Forced tween animation instead of spring physics to avoid hitching on low-end devices, (6) Added explicit translateZ(0) to force GPU layer compositing, (7) Reduced intersection margin from -100px to -50px for earlier trigger. Testing on mobile (375x667) and desktop (1920x1080) confirmed smooth, fluid animations with no stuttering or jumping. All card slide-ins now glide smoothly into position on both mobile and desktop.
